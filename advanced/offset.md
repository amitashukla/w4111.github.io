...
## Add OFFSET to the LIMIT Clause
...
You will edit `parse_sql.py` in two places.  You will edit the PEG grammar to support the "OFFSET expression" syntax.   OFFSET is an optional part of the LIMIT clause.  If OFFSET is specified, then it should be followed by one or more white spaces, followed by an expression that evaluates to a number.  The expression cannot reference any attributes.
...
At this point, run `python test_offset.py` and all the tests should pass.
